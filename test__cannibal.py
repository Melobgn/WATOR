...
class Monde:
    ...
    def peupler_le_monde(self, nb_poissons, nb_requins):
        ...
        for _ in range(self.nb_poissons):
            ...
            self.grille[x][y] = '\U0001f41f' #poisson
        ...
        for _ in range(self.nb_requins):
            if not coordonnees_possibles:
                break
            x, y = coordonnees_possibles.pop()
            self.grille[x][y] = '\U0001f988' #requin
...
class Requin(Poisson):
    ...
    #Ajouter une fonctionnalité, quand les requins sont proches de la mort, ils deviennent cannibales.

    def cannibalisme(self):
        if self.energie < 2:
            for requinx in self.monde.requins:
                if (abs(self.x - requinx.x) <= 1 and abs(self.y - requinx.y) == 0) or (abs(self.x - requinx.x) == 0 and abs(self.y - requinx.y) <= 1):
                    self.monde.grille[requinx.x][requinx.y] = 0
                    self.energie += 2
                    self.monde.requins.remove(requinx)
...
while chronons < 50:
    ...
    # Faire se déplacer chaque poisson
    for poiscaille in mon_monde.poissons:
        poiscaille.deplacer_poisson()

    # Faire se déplacer chaque requin
    for requinx in mon_monde.requins:
        requinx.starvation()
        requinx.deplacer_requin()
        requinx.manger_poisson()
        requinx.cannibalisme()

    for poiscaille in mon_monde.poissons:
        mon_monde.grille[poiscaille.x][poiscaille.y] = '\U0001f41f' #poisson

    for requinx in mon_monde.requins:
        mon_monde.grille[requinx.x][requinx.y] = '\U0001f988' #requin
